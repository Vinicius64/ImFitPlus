...
class UserRepository(context: Context) {
    ...
    fun insert(user: User): Long {
        ...
        val id = db.insert("user_event", null, cv)
        db.close()
        return id
    }
    ...
    fun update(user: User): Int {
        ...
        val rows = db.update("user_event", cv, "id = ?", arrayOf(user.id.toString()))
        db.close()
        return rows
    }
    ...
    fun getById(id: Long): User? {
        ...
        val cursor = db.query("user_event", null, "id = ?", arrayOf(id.toString()), null, null, null)
        var user: User? = null
    ...
    fun getLast(): User? {
        val db = dbHelper.readableDatabase
        val cursor = db.query("user_event", null, null, null, null, null, "id DESC", "1")
        var user: User? = null
        if (cursor.moveToFirst()) {
            user = User(
                id = cursor.getLong(cursor.getColumnIndexOrThrow("id")),
